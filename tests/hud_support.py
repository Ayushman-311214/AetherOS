...
Nothing here touches Qt, a display, or a subprocess: the point is that the
service layer is fully exercisable on a headless machine.

Lives at the tests root rather than under ``tests/hud/`` because the bootstrap
wiring tests need the same double — and since ``tests/`` has no ``__init__.py``,
a sibling package's module cannot be imported. Only the two conftests import
this module; test files ask for the ``fake_hud_process`` fixture instead.
"""
...
from aetheros.hud.protocol import Message, ready_message, stats_message
